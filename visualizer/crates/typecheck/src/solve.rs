use crate::{
    Bound, Constraint, Group, GroupKey, GroupKeys, Instantiation, Substitutions, Ty, TyGroups,
};
use ena::unify::InPlaceUnificationTable;
use std::{cell::RefCell, collections::BTreeMap, fmt::Debug, mem, rc::Rc};
use visualizer_db::{Db, Fact, NodeId};
...
impl<'a> Solver<'a> {
    ...
    pub fn insert(&mut self, constraints: impl IntoIterator<Item = Constraint>) {
        self.queue.extend(constraints);
        self.run();
    }

    pub fn finish(&self) -> TyGroups {
        ...
        let typed_nodes = db
            .nodes()
            .filter(|&node: &NodeId| !db.is_hidden(node) && db.get::<()>(node, "untyped").is_none())
            .collect::<Vec<_>>();

        for node in typed_nodes {
            // Give untyped expressions a default type of `Unknown`
            if ty_groups.index_of(node).is_none() {
                db.fact(node, Fact::new("unknownType", ()));
            ...
            for ty in tys {
                ...
                db.fact(node, Fact::new("type", ty.clone()));
            }

            if all_incomplete {
                db.fact(node, Fact::new("incompleteType", ()));
            }
        }
...
impl Solver<'_> {
    ...
    fn run_bounds(&mut self) -> Progress {
        ...
        for bound in bounds {
            ...
            let instances = self.get_trait_instances(bound.tr);
            ...
            if candidates.len() != 1 {
                self.flag_unresolved(temp_node, bound);
                continue;
            }
            ...
            *self = copy;
            ...
            self.progress.set();
            self.flag_resolved(temp_node, bound, instance);
        }
...
impl Solver<'_> {
    fn get_trait_instances(&mut self, trait_id: NodeId) -> Vec<(NodeId, Substitutions)> {
        self.db
            .borrow()
            .iter_by(trait_id, "instance")
            .map(|&node| {
                (
                    node,
                    self.db
                        .borrow()
                        .get::<Substitutions>(node, "substitutions")
                        .unwrap()
                        .clone(),
                )
            })
    ...
    fn flag_resolved(&mut self, node: NodeId, bound: Bound, instance: NodeId) {
        let mut db = self.db.borrow_mut();
        db.fact(node, Fact::new("resolvedTrait", bound.tr));
        db.fact(node, Fact::new("resolvedTrait", instance));
    }

    fn flag_unresolved(&mut self, node: NodeId, bound: Bound) {
        self.db
            .borrow_mut()
            .fact(node, Fact::new("unresolvedTrait", bound.tr));
    }
}
